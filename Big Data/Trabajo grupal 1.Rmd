---
title: "Trabajo grupal 1 - Sampling Data"
author: "Calvo, Andrea; Ibañez, Mauricio; Lévano, Valeria; Núñez, César"
output:
...
```{r}
#Cambiar a donde tengan almacenada la base de datos
setwd("C:/Users/User/Documents/GitHub/Diplomado_PUCP/Big Data")
```
...
#Seleccioamos al 80% de cluster por cada estrato
set.seed(20176040)
mac1_clusters <- data1 %>%
   group_by(estrato) %>%
...
muestra <- mac1_f %>%
  ...
  sample_n(if_else(n() <= 13, 1, round(n() * 0.05)))
```
...
muestra <- muestra %>% 
  mutate(pwe = case_when(
      estrato.x == 1 ~ (799/19573)*p2[1],
      estrato.x == 2 ~ (1373/34232)*p2[2],
      estrato.x == 3 ~ (901/22375)*p2[3]), .keep = "all")
...
La Encuesta Demográfica y de Salud Familiar - ENDES provee información actualizada sobre la dinámica demográfica, el estado de salud de las madres y niños menores de cinco años, así como también brindar información sobre el estado y factores asociados a las enfermedades no transmisibles y transmisibles, así como el acceso a servicios de diagnóstico y tratamiento. 

Para la selección de la muestra se utilizó la información estadística y cartográfica proveniente de los Censos Nacionales XII de Población y VII de Vivienda del año 2017 (CPV 2017), y el material cartográfico actualizado para tal fin en el proceso de actualización cartográfica realizado para la ejecución de la ENDES.

La unidad primaria de muestreo (UPM) es el conglomerado La unidad secundaria de muestreo (USM) corresponde a las viviendas particulares
...
2.	Elegir y calcular dos variables: (i) una dicotómica, (ii) una continua. Por lo menos la variable dicotómica debe ser una variable definida y creada por ustedes. (2 puntos)
